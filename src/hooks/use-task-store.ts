...
const useTaskStore = () => {
  ...
  const [focusedTask, setFocusedTask] = useLocalStorage<Task>('focused',tasks.filter( task =>  task.isComplete === false)[0]);
  ...
  const api = {tasks, setTasks, updateTaskCompletion, focusedTask, shuffleFocusedTask}; 
